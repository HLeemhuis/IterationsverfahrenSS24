\section{Konvergenzanalyse für GMRES/MINRES}

Gegeben sei $A\in\cnn,\ Ax=b,\ A$ regulär.
...
\item Ist $A$ normal, dann ist $F(A)=$co(spek$(A)$) (co$(M)$ = konvexe Hülle von $M$).
\end{enumerate}
\end{lem}
...
\begin{enumerate}
\item Klar.
\item Siehe \textsc{Roger A. Horn, Charles R. Johnson}, Topics in Matrix Analysis, Cambridge Univ Press; (1994)
      (behandelt gerade $F(A)$).
...
Schränken wir $A\in\co^{n\times n}$ auf einen Unterraum $V \subseteq \co^n$ ein, so ist nicht
notwendig $AV \subseteq V$. Möchte man einen Homomorphismus auf $V$, muss man $AV$ geeignet
projizieren.
...
Vorüberlegung.
Nach Lemma~\ref{minimierungs_lem} gilt für $\frac{1}{\xi} =
...
\end{align*}
Also ist für die angegebene Wahl von $\xi$
...
\begin{Blist}{$m-1\to m$}
\item[\hfill $m=0$] klar.
\item[$m-1\to m$] Für $w=p_{m-1}(A)r^0$ ist nach Induktionsvoraussetzung
\[
\|w\|_2\le \left(1-\textstyle\frac{c^2}{\|A\|_2^2} \right)^{\textstyle\frac{m-1}{2}}\cdot \|r^0\|_2.
\]
Auf Grund der Minimalitätseigenschaft ist für alle $\xi\in\co\backslash\{0\}$
\[
\|p_m(A)r^0\|_2\le \|(I-\textstyle\frac{1}{\xi}A) p_{m-1}(A)r^0\|_2 = \|(I-\textstyle\frac{1}{\xi}A)w\|_2.
\]
Speziell für die Wahl $\textstyle\frac{1}{\xi}=\textstyle\frac{\langle w,Aw\rangle}{\langle Aw,Aw\rangle}$ ergibt sich nach \eqnref{absch1_eq}
\[
...
\begin{bem}Der Beweis hat gezeigt, dass unter den Voraussetzungen von Satz \ref{GMRESabschaetz_sa} sogar gilt, dass GMRES($k$) für alle Restartwerte
$k$  mit der selben Abschätzung \eqnref{gmres_konv_sa} konvergiert.
\end{bem}

Wir diskutieren die Nullstellen der GMRES-Polynome jetzt noch genauer.

\begin{lem}\label{GMRES_res_lem}
Für die GMRES-Residuen
\[
...
\]
Also erfüllt $z^m$ die Normalengleichung (s.\ Numerik I)
\[
...
z^m=M_m^{-1}V_m^HAr^0
\]
und es gilt für alle $y\in\co^{m}$
\begin{align*}
...
\end{equation}
wobei $H_m$ aus $H_{m+1,m}$ durch Streichen der letzten Spalte entsteht.
\end{sa}
\begin{proof}
Man beachte, dass das Produkt $H_{m+1,m}^HH_{m+1,m}$  hpd ist, $H_m$ jedoch singulär sein kann.
Sei 
\[
...
\]
Die orthogonale Projektion von $A^{-1}$ auf $AK_m(A,r^0)$ wird repräsentiert durch
\begin{align*}
