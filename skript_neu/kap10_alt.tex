\section[CG und BiCG]{CG und BiCG}
{\bf Gegeben: }
\begin{quote}
$Ax=b$, $A \in \cnn$, $A$ regulär.
\end{quote}
{\bf Gesucht: }
\begin{quote}
...
\end{defn}
Ab jetzt diskutieren wir BiCG. F"ur CG siehe Vorlesung "`Parallele Algorithmen WS 2005/06"'.
\begin{lem}
...
Wie bei CG (siehe Vorlesung Parallele Algorithmen WS 2005/06) kann man aus dem unsymmetrischen Lanzcos-Verfahren
eine Standardform des BiCG-Verfahrens herleiten. Wir geben hier lediglich das Resultat an:

\begin{alg}[BiCG, Fletcher 1973]
~               % um "Algorithmus" aus dem Kasten rauszubekommen
\vspace*{-2\baselineskip}       % um den Leeraum zu entfernen
\begin{algorithm}
  \begin{algorithmic}
    \STATE w\"ahle $x^0$, setze $r^0 = b-Ax^0$, wähle $\tilde{r}^0,\ p^0=r^0,\ \tilde{p}^0=\tilde{r}^0$
    \FOR{$k = 0,1, \dots$ }
...
\item Breakdowns: Lanczos Breakdown (erkennbar durch $\langle r^k, \tilde{r}^k \rangle=0$) und BiCG-Breakdown
			($T_k$ ist singulär, d.h. es existiert keine Iterierte mit $r^k \bot K_k(A^H,\tilde{r}^0)$,
			erkennbar durch $\langle Ap^k, \tilde{p}^k \rangle =0$).
\end{enumerate}
\end{bem}

\begin{aufg}
Führe BiCG für das Modellproblem III durch.
\end{aufg}
...
\end{enumerate}
Zeige jeweils: Für geeignete Wahl von $\tilde{r}^0$ (bzw. $w^0$) kann man den unsymmetrischen Lanczos-Prozess mit nur einer MVM mit $A$
(und eventuell einer zusätzlichen mit $J$) durchführen
(dies überträgt sich dann auch auf BiCG).
\end{aufg}
