...
\begin{defn} \label{def:Tschebyscheff-Polynom}
...
Mit $E=E(f_1,f_2,\hat{\rho})$, $a=\frac{1}{2}(f_2-f_1)$ und
...
\begin{bem}
  ...
  deshalb ist es aus numerischen Gründen besser, die Berechnung von
  $c_{m+1}$ zu vermeiden, und dafür die Quotienten $\delta_{m+1}=
...
zur Lösung von $Ax = c$ ($b$ ist schon vergeben \ldots):
\clearpage
...
\begin{aufg}
  Implementiere das Tschebyscheff-Verfahren für das Modellproblem~I
  für verschiedene Gittergrößen $N$.
  ...
  \begin{align*}
    ...
    \Xi &= 4+4\cos(\pi h).
  \end{align*}
  Die zugehörige Ellipse ist also
  \begin{equation*}
    E(\xi,\Xi,1)
  \end{equation*}
  \begin{itemize}
  \item Plotte $\norm{r^m}$ gegen $m$ für verschiedene Werte von $N$.
